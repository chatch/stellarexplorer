export default {
  NaoBTC: {
    home: 'naobtc.com',
    currencies: {
      BTC: {
        issuer: 'GATEMHCCKCY67ZUCKTROYN24ZYT5GK4EQZ65JJLDHKHRUZI3EUEKMTCH',
        distributers: [
          'GBTCTEAJHSKZUVSOX2QJ42FFI5GE6ECNR4IFOTCAMA5Z7W24JFR3VTK5',
        ],
      },
      XEL: {
        issuer: 'GAXELY4AOIRVONF7V25BUPDNKZYIVT6CWURG7R2I6NQU26IQSQODBVCS',
        distributers: [
          'GCINGGW2VWB7XOWCZVJ3XEKOX5RH2ZYT7RKRX6PRV7LYH7K2YLO6SMFI',
        ],
      },
    },
  },
  TEMPO: {
    home: 'tempo.eu.com',
    currencies: {
      EURT: {
        issuer: 'GAP5LETOV6YIE62YAM56STDANPRDO7ZFDBGSNHJQIYGGKSMOZAHOOS2S',
        distributers: [
          'GC2BQYBXFOVPRDH35D5HT2AFVCDGXJM5YVTAF5THFSAISYOWAJQKRESK',
        ],
      },
    },
  },
  eQuid: {
    home: 'equid.co',
    currencies: {
      EQD: {
        issuer: 'GCGEQJR3E5BVMQYSNCHPO6NPP3KOT4VVZHIOLSRSNLE2GFY7EWVSLLTN',
        distributers: [
          'GAZTZ2T322AFOONMXBN5P3MILKX7QS5BPKHZZVPUXZLCQ6JPBWMDGH5P',
        ],
      },
    },
  },
  RippleFox: {
    home: 'ripplefox.com',
    currencies: {
      CNY: {
        issuer: 'GAREELUB43IRHWEASCFBLKHURCGMHE5IF6XSE7EXDLACYHGRHM43RFOX',
        distributers: [
          'GBK4DFCUAZRNU7TJ4XUOJEADVQBLGVVVFKRTHHXNAXD7MTYUWR7HKCNY',
        ],
      },
    },
  },
  'coins.ph': {
    home: 'coins.ph',
    currencies: {
      PHP: {
        issuer: 'GBUQWP3BOUZX34TOND2QV7QQ7K7VJTG6VSE7WMLBTMDJLLAW7YKGU6EP',
        distributers: [
          'GA6KWMT33N63HIV2NZ3AFWENTYI4ZJSNAXQMTQI3XJK5GORN36SETSNF',
          'GCKX3XVTPVNFXQWLQCIBZX6OOPOIUT7FOAZVNOFCNEIXEZFRFSPNZKZT',
        ],
      },
    },
    img: 'coinsph.png',
    toml: 'https://coins.asia/.well-known/stellar.toml',
  },
  VCBear: {
    home: 'vcbear.net',
    currencies: {
      JPY: {
        issuer: 'GBVAOIACNSB7OVUXJYC5UE2D4YK2F7A24T7EE5YOMN4CE6GCHUTOUQXM',
        distributers: [
          'GCGEQJR3E5BVMQYSNCHPO6NPP3KOT4VVZHIOLSRSNLE2GFY7EWVSLLTN',
        ],
      },
      XRP: {issuer: 'GA7FCCMTTSUIC37PODEL6EOOSPDRILP6OQI5FWCWDDVDBLJV72W6RINZ'},
      BTC: {issuer: 'GDXTJEK4JZNSTNQAWA53RZNS2GIKTDRPEUWDXELFMKU52XNECNVDVXDI'},
    },
  },
  TONAIRA: {
    home: 'tonaira.com',
    currencies: {
      NGN: {
        issuer: 'GCLRUZDCWBHS7VIFCT43BARPP63BHR32HMEVKXYQODA5BU6SIGFK4HL2',
        distributers: [
          'GCZAGTNARIVAGJG5MMQSBCO5OHDAFDBONJVSVK3TIOEOV3V3LPGYXEC7',
        ],
      },
      TICO: {
        issuer: 'GCLRUZDCWBHS7VIFCT43BARPP63BHR32HMEVKXYQODA5BU6SIGFK4HL2',
        distributers: [
          'GBB3CHRY4TV6MEICZKAHNAOCF6CA7BGJWKCKC7FUDI42RI5AYCKHB3PI',
        ],
      },
    },
  },
  Papaya: {
    home: 'btc.papayame.com',
    currencies: {
      BTC: {
        issuer: 'GAUTUYY2THLF7SGITDFMXJVYH3LHDSMGEAKSBU267M2K7A3W543CKUEF',
        distributers: [
          'GCR3L4I5C2WE4C5BNHEFHLFSIYHGLZRDAUG3MOWL7R3CVVSI4XL47OTJ',
        ],
      },
      LTC: {
        issuer: 'GC5LOR3BK6KIOK7GKAUD5EGHQCMFOGHJTC7I3ELB66PTDFXORC2VM5LP',
        distributers: [
          'GA7XRBMPWWIGOINPHEFLF2JMWKDP2ZAXNAWI6LWOWO4WWPFLEHNJWL5Y',
        ],
      },
      ICN: {
        issuer: 'GBDEVU63Y6NTHJQQZIKVTC23NWLQVP3WJ2RI2OTSJTNYOIGICST6DUXR',
        distributers: [
          'GDBWXSZDYO4C3EHYXRLCGU3NP55LUBEQO5K2RWIWWMXWVI57L7VUWSZA',
        ],
      },
    },
    toml: 'https://btc.papayame.com/.well-known/stellar.toml',
  },
  StellarTerm: {
    home: 'stellarterm.com',
    currencies: {
      DEMO: {
        issuer: 'GBAMBOOZDWZPVV52RCLJQYMQNXOBLOXWNQAY2IF2FREV2WL46DBCH3BE',
      },
    },
  },
}
