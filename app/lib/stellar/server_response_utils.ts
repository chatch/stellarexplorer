 ...
 */
const serverApiResponseToState = (rsp: ServerApi.CollectionPage<
...
const ledgerRspRecToPropsRec = (rspRec: ServerApi.LedgerRecord): LedgerProps => {
    ...
    return {
        ...
        baseReserve: baseFeeInStroops ? rspRec.base_reserve_in_stroops : Number(rspRec.base_reserve),
...
const transactionRspRecToPropsRec = (rspRec: ServerApi.TransactionRecord): TransactionProps => ({
    id: rspRec.id,
    hash: rspRec.hash,
...
const keysToCamelCase = (rec: Record<string, any>) => mapKeys(rec, (v, k) => camelCase(k))

const keysToCamelCasePlusTime = (rec: Record<string, any>, timePropKey = 'created_at'): Record<string, any> & { time: string } => ({
